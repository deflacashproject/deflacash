...
#include "config/deflacash-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("deflacash-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "deflacash.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "deflacashd.pid"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, deflacash, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all DFC specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizedeflacashamount=<n>", strprintf(_("Keep N PIV anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The deflacash Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("deflacash-loadblk");
...
/** Sanity checks
 *  Ensure that deflacash is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. deflacash Core is shutting down."));
...
#endif
    // Make sure only a single deflacash process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. deflacash Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("deflacash version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of deflacash Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart deflacash Core to complete") << "\n";
    ...
    nAnonymizeDeflacashAmount = GetArg("-anonymizedeflacashamount", 0);
    if (nAnonymizeDeflacashAmount > 999999) nAnonymizeDeflacashAmount = 999999;
    if (nAnonymizeDeflacashAmount < 2) nAnonymizeDeflacashAmount = 2;
    ...
    LogPrintf("Anonymize deflacash Amount %d\n", nAnonymizeDeflacashAmount);
    ...
    /* Denominations
       ...
       For example:
       1DFC+1000 == (.1DFC+100)*10
       10DFC+10000 == (1DFC+1000)*10
    */
